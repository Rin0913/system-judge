...
def update_problem(problem_id):
    ...
    problem_id = g.problem_repository.update_problem_details(problem_id,
                                                             problem_name,
                                                             start_time,
                                                             deadline)
    return jsonify({'problem_id': problem_id})
...
def query_problem(problem_id):
    problem_data = g.problem_repository.query_problem(problem_id)
    return jsonify(problem_data)

@problem_bp.route('/list', methods=['GET'])
def list_problems():
    problem_data = g.problem_repository.list_problems()
    return jsonify(problem_data)

@problem_bp.route('/<string:problem_id>', methods=['DELETE'])
def del_problem(problem_id):
    problem_id = g.problem_repository.del_problem(problem_id)
    return jsonify({'problem_id': problem_id})
